...
/** Verse counts in canonical chapter order. This preserves index alignment for
 * Strong's occurrences and semantic vectors without bundling Scripture text. */
...
/** Every verse key in the canonical order used by local derived indexes. */
export function allVerseKeys(): string[] {
  return BOOKS.flatMap((book) =>
    (VERSE_COUNTS[book.abbrev] ?? []).flatMap((count, chapterIndex) =>
      Array.from({ length: count }, (_, verseIndex) =>
        `${book.abbrev}.${chapterIndex + 1}.${verseIndex + 1}`
      )
    )
  );
}
