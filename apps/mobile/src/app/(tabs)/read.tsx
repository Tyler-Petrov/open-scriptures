...
import { BOOKS, getBookMeta } from "@/lib/bible";
...
export default function ReadScreen() {
  ...
  const chapterIntro = chapterState.status === "ready" ? chapterState.data.intro : undefined;
  const attribution =
    chapterState.status === "ready" && !tInfo.offline ? chapterState.data.copyright : null;
  useEffect(() => {
  ...
  // Audio is aligned to the KJV; leaving it ends the session.
  useEffect(() => {
    if (isKjv || !audioUrl) return;
    // eslint-disable-next-line react-hooks/set-state-in-effect -- tear down the player when the text it narrates is no longer shown
    stopAudio();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isKjv]);
    ...
    <View style={styles.root}>
      <View style={styles.header}>
        ...
        <Pressable
          ...
          accessibilityRole="button"
          accessibilityLabel={`Translation: ${tInfo.name}`}
          style={({ pressed }) => [styles.versionPill, pressed && { opacity: 0.7 }]}
        >
          <Text style={styles.versionText}>{translation}</Text>
        </Pressable>
      ...
      {loading ? (
        ...
        // Never swap in another translation silently: say what's wrong.
        <View style={styles.center}>
          <Icon name="info" size={28} color={c.subtext} />
          <Text style={styles.errorTitle}>{tInfo.name} isn’t available right now</Text>
          <Text style={styles.errorBody}>{chapterState.message}</Text>
          <GhostButton label="Try again" onPress={retryChapter} />
          <GhostButton label="Choose another translation" onPress={() => setTranslationOpen(true)} />
        </View>
      ) : (
        <FlatList
          ref={listRef}
          ...
          ListHeaderComponent={
            <View style={styles.heading}>
              ...
              {chapterIntro ? <Text style={styles.intro}>{chapterIntro}</Text> : null}
            </View>
          }
          ListFooterComponent={
