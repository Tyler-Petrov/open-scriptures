...
export default function VerseSheet({ verseKey, text, translation, onClose }: VerseSheetProps) {
  const { c } = useTheme();
  ...
  // Shared text names the translation; licensed ones carry their attribution.
  const tInfo = TRANSLATIONS[translation];
  const quoted =
    `“${text}” — ${ref(verseKey)} (${translation})` +
    (tInfo.offline ? "" : `\n${tInfo.shortAttribution}`);
    ...
    >
      ...
      >
            <View style={styles.refRow}>
              ...
              <Text style={styles.version}>{translation}</Text>
