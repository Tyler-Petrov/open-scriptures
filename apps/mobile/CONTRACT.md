...
| Key | Shape |
|---|---|
| `openscripture.settings` | `{ theme: "system"\|"light"\|"dark", fontSize: number, onboarded: boolean }` |
| `openscripture.position` | `{ book: string (abbrev), chapter: number }` — last reading position |
| `openscripture.highlights` | `{ [verseKey]: "yellow"\|"green"\|"blue"\|"pink"\|"purple" }` |
| `openscripture.bookmarks` | `string[]` (verseKeys, ordered) |
| `openscripture.notes` | `{ [verseKey]: string }` |
| `openscripture.planProgress` | `{ [planId]: { completed: number[] } }` |
...
- `src/lib/theme.tsx`: `ThemeProvider`, `useTheme()` → `{ c: {bg, card, line, text, subtext, gold, ox}, isDark }`.
  Follows `openscripture.settings.theme` ("system" listens to useColorScheme). Provide `LIGHT`/`DARK` palettes per design language.
...
- S-02 Home: Verse of the Day card (deterministic per date: e.g. verseKey = dayOfYear rotation across a curated list or Psa/Pro/Jhn pool) → "Read in context" → reader; Continue Reading card from `openscripture.position` with progress bar (book/chapter of 1,189 chapters); quick chips: Search, Plans, Random verse.
